...
const fitsInOneBox = (boxes) => {
  let boxFit = []
  let cantfit = []
  boxes.forEach((box1,idx) => {
      for (let i = 0; i < boxes.length; i++) {
        if(i !== idx){
            boxFit.push(box1.l < boxes[i].l && box1.w < boxes[i].w && box1.h < boxes[i].h)
        }
      }
      cantfit.push(boxFit)
      boxFit = []
  ...
  let trueCant = 0
  cantfit.forEach(arr => {
    allFalseCant += arr.every(i => i === false)
    allTrueCant += arr.every(i => i)
    trueCant += arr.some(i => i)
  })

  return allTrueCant === 1 && allFalseCant === 1 && trueCant === boxes.length-1;
}
